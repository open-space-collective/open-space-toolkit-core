...
std::ostream& operator<<(std::ostream& anOutputStream, const File& aFile)
{
        ...
        switch (errno)
        {
            case EACCES:
                throw ostk::core::error::RuntimeError(
                    ...
                    "of the directories in the path prefix of [{}].",
                    aPath.toString()
                );

            case ELOOP:
                throw ostk::core::error::RuntimeError(
                    "[ELOOP] Too many symbolic links were encountered in resolving [{}].", aPath.toString()
                );
            ...
            case ENOENT:
                throw ostk::core::error::RuntimeError(
                    "[ENOENT] A component of [{}] does not exist or is a dangling symbolic link.", aPath.toString()
                );

            case ENOTDIR:
                throw ostk::core::error::RuntimeError(
                    "[ENOTDIR] A component used as a directory in [{}] is not, in fact, a directory.", aPath.toString()
                );
            ...
            case EFAULT:
                throw ostk::core::error::RuntimeError(
                    "[EFAULT] Path [{}] points outside your accessible address space [{}].", aPath.toString()
                );

            case EINVAL:
                throw ostk::core::error::RuntimeError(
                    "[EINVAL] Mode was incorrectly specified [{}].", aPath.toString()
                );
            ...
            case ENOMEM:
                throw ostk::core::error::RuntimeError(
                    "[ENOMEM] Insufficient kernel memory was available [{}].", aPath.toString()
                );
